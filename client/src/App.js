...
function App() {
  ...
  const catDeleteHandler = async (cat) => {
    console.log("CAT: ", cat);
    const response = await axiosPrivate.delete("cats/", {
      data: JSON.stringify(cat.id),
    });
    console.log(response.data);
    getCats(catUrl);
  };
    ...
    <Routes>
      <Route path="/" element={<Layout />}>
        ...
        <Route element={<RequireAuth allowedRoles={[ROLES.Editor]} />}>
          ...
          <Route
            path="dogs/delete/:id"
            element={<DogDelete deleteHandler={dogDeleteHandler} />}
          />
          <Route path="cats" element={<Cats cats={cats} getCats={getCats} />} />
          <Route
            path="cats/create"
            element={<CatAdd addHandler={catAddHandler} />}
          />
          <Route path="cats/view/:id" element={<CatDetail />} />
          <Route
            path="cats/edit/:id"
            element={<CatEdit updateHandler={catUpdateHandler} />}
          />
          <Route
            path="cats/delete/:id"
            element={<CatDelete deleteHandler={catDeleteHandler} />}
          />
        </Route>
